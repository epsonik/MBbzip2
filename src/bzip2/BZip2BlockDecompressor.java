...
public class BZip2BlockDecompressor {
    ...
    /* Move To Front stage */
    /**
     * Counts of each byte value within the {@link bwtTransformedArray} data.
     * Collected at the Move To Front stage, consumed by the Inverse Burrows
     * Wheeler Transform stage
     */
    ...
    /**
     * At each position contains the union of :- An output character (8 bits) A
     * pointer from each position to its successor (24 bits, left shifted 8
     * bits) As the pointer cannot exceed the maximum block size of 900k, 24
     * bits is more than enough to hold it; Folding the character data into the
     * spare bits while performing the inverse BWT, when both pieces of
     * information are available, saves a large number of memory accesses in the
     * final decoding stages.
     */
     ...
     * @throws IOException if the input stream reaches EOF before all table data
     * has been read
     */
     ...
     * @throws IOException if an end-of-block symbol was not decoded within the
     * declared block size
     */
     ...
     * randomisation applied, reverses the randomisation
     *
     ...
     * byte from the Burrows-Wheeler Transform stage when required
     *
     ...
     * new bytes from the Burrows-Wheeler Transform stage when required
     *
     * @param destination The array to write to
     * @param offset The starting position within the array
     * @param length The number of bytes to read
     * @return The number of bytes actually read, or -1 if there are no bytes
     * left in the block
     */
    ...
    /**
     * Verify and return the block CRC. This method may only be called after all
     * of the block's bytes have been read
     *
